...
def load_data_from_bigwig(bigwig_path, fasta_path, chromosome="chr19",start=0,end=1000000):
    """
...
def main():
    ...
    parser.add_argument('--bw', required=True, help='true bw file path')
    parser.add_argument('--fasta', required=True, help='Reference genome fasta file path')
    ...
    df['parsed_true_expression'] = 0
    df['calculated_length'] = df['end'] - df['start']
    df['parsed_length'] = df['parsed_expression'].apply(len)

    df['parsed_true_expression'] = df.apply(
        lambda row: load_data_from_bigwig(
            args.bw, 
            args.fasta,
            row['chromosome'], 
            row['start'], 
