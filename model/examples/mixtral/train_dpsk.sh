...
DATA_PATH=/mnt/zzb/Public/DataSet/human_genome_assemblies/demo_data/BPE/test/HG005.bpe_4k.32k_fixlen_text_document
TOKENIZER_PATH=/mnt/zzb/Public/DataSet/human_genome_assemblies/preprocess/bpe_tokenizer_training/round1.full.4096.model
...
MODEL_PARALLEL_ARGS=(
    ...
    --distribute-saved-activations ##not compatable with sequence parallel
##    --sequence-parallel
...
if [ $FL = true ]; then
    ...
    ##fl_options=" --attention-backend fused " ###VDisabling FlashAttention as it does not support MLA
    export NVTE_FLASH_ATTN=1 NVTE_FUSED_ATTN=0
    fl_options=" --attention-backend fused "
    echo $fl_options
elif [ $FL = false ]; then
