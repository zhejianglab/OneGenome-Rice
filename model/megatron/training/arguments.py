...
def validate_args(args, defaults={}):
    ...
    #         '--mrope-section should be set when using --position-embedding-type mrope.'
     ...
     # Allow yarn with flash_attn
        if not args.multi_latent_attention and args.rope_type != 'rope':
            if args.use_flash_attn:
                print(f"Warning: Using {args.rope_type} with flash attention. This is experimental.")
            else:
                assert False, f'Common attention only support rope_type="rope", but got {args.rope_type}.'
...
def _add_data_args(parser):
    ...
    group.add_argument('--loss-mask-tokens', nargs='+', type=str, default=None,
                       help='List of token strings to mask from loss calculation. '
                       'For example: --loss-mask-tokens N A T G C')
