...
class GPTDatasetConfig(BlendedMegatronDatasetConfig):
    ...
    loss_mask_tokens: Optional[Tuple[int, ...]] = None
    """Tuple of token IDs to mask from loss calculation"""
...
def _get_ltor_masks_and_position_ids(
    ...
    # Mask specified tokens from loss calculation
    if loss_mask_tokens:
        for token_id in loss_mask_tokens:
            mask_count = (data == token_id).sum().item()
            if mask_count > 0:
                log_single_rank(
                    logger,
                    logging.INFO,
                    f"Masking token_id={token_id}: {mask_count} positions masked out of {seq_length}",
                )
            loss_mask[data == token_id] = 0.0
        total_masked = (loss_mask == 0.0).sum().item()
        log_single_rank(
            logger,
            logging.INFO,
            f"Total masked positions: {total_masked}/{seq_length} ({100*total_masked/seq_length:.2f}%)",
        )
