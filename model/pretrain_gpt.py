...
def core_gpt_dataset_config_from_args(args):
    ...
    # Get token IDs to mask from loss calculation
    loss_mask_tokens = None
    if getattr(args, 'loss_mask_tokens', None):
        # args.loss_mask_tokens can be a list of token strings like ['N', 'A', 'T', 'G', 'C']
        token_ids_list = []
        for token_str in args.loss_mask_tokens:
            token_ids = tokenizer.tokenize(token_str)
            token_ids_list.extend(token_ids)
        loss_mask_tokens = tuple(token_ids_list)
        print_rank_0(f"> Loss mask tokens: {args.loss_mask_tokens} -> token IDs: {loss_mask_tokens}")
